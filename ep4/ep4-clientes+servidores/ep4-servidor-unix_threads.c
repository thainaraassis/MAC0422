/* Servidor de echo usando sockets de domínio Unix e múltiplas threads
...
int main() {
    ...
    if (unlink(SOCKET_NAME)==-1)
        if (errno != ENOENT) {
            syslog(LOG_ERR|LOG_USER,"Erro no unlink :-( [%m]\n");
            exit(8);
        }
    ...
    syslog(LOG_INFO|LOG_USER,"Servidor de echo no ar! no socket local %s :-)\n", SOCKET_NAME);
